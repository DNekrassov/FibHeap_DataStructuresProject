...
class HeapTest {
    ...
    void testDeleteMin() throws InterruptedException {
        ...
        assert (heap.getTreeNum() == 16) : "number of internal trees isn't as expected";

        heap.deleteMin();

        assert (heap.findMin() != null);
        assert (heap.findMin().getKey() == 1) : "minimal key isn't as expected after deleteMin";
        assert (heap.getTreeNum() == 4) : "number of internal trees isn't as expected after deleteMin";
        ...
        for (int tries = 1; tries < 5; tries++) {
            ...
            while (!heap.isEmpty()) {
                ...
                heap.deleteMin();
                assert (heap.getTreeNum() == expectedNumOfTrees) : "number of internal trees isn't as expected after deleteMin";
                numOfKeys -= 1;
            }
        }
    ...
    void testTotalCuts() {
        ...
        assert (FibonacciHeap.cuts == 0) : "reset cuts";
        ...
        assert (FibonacciHeap.cuts == 1) : "incorrect cuts count";

        heap.decreaseKey(nodes[6], 2);

        assert (FibonacciHeap.cuts == 3) : "incorrect cuts count";

        heap.decreaseKey(nodes[3], 3);
        assert (FibonacciHeap.cuts == 4) : "incorrect cuts count";
    ...
    void testMeld() {
        ...
        assert (heap1.getTreeNum() == 11) : "incorrect number of trees";
        ...
        assert (heap1.getTreeNum() == 8) : "incorrect number of trees";
