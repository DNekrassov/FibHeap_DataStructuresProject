...
public class FiboHeapTest {
    ...
    public static int getNumMarked(FibonacciHeap heap) {
        return (heap.potential() - heap.getTreeNum()) / 2;
    }
...
//                System.out.println("num of trees is" + heap.getNumOfTrees());
//                FiboHeapPrinter.printHeap(heap);
    ...
    public static void testMeld(FibonacciHeap heap, int upperBound) {
        ...
        int size = heap.size(), numOfTrees = heap.getTreeNum(), numMarked = getNumMarked(heap);
        heap.meld(heap2);
        if (heap.size() != size + heap2.size()) {
            throw new RuntimeException("size after meld is not sum of both heaps");
        }
        if (heap.getTreeNum() != numOfTrees + heap2.getTreeNum()) {
    ...
    public static void testKMin(FibonacciHeap heap) {
        if (heap.size() == 0) {
            ...
            FibonacciHeap heaps[] = new FibonacciHeap[heap.getTreeNum()];
            int heapIndex = 0;
            do {
                next = cur.getNext();
                cur.setPrev(cur);
                cur.setNext(cur);
                //heaps[heapIndex] = new FibonacciHeap(cur, countTreeItems(cur)); - original line - have no idea what is this constructor
    ...
    public static void testNumOfTrees(FibonacciHeap heap) {
        ...
        if (actualNumOfTrees != heap.getTreeNum()) {
            throw new RuntimeException(String.format("should have %s trees but counted %s", heap.getTreeNum(), actualNumOfTrees));
        }
    }
    ...
    public static void testHeapFields(FibonacciHeap heap, ExpectedFields expected) {
        if (expected.numOfTrees != null && expected.numOfTrees != heap.getTreeNum()) {
            throw new RuntimeException(
                    String.format("wrong amount of trees. found %d but expected %d",
                            heap.getTreeNum(), expected.numOfTrees)
            );
        }
